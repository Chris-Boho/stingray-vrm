...
import { useMousePositionStore } from '../../stores/mouseStore';
...
export const CanvasContainer: React.FC<CanvasContainerProps> = ({ className = '' }) => {
		...
		return (
			<div className={`flex items-center justify-center h-full ${className}`}>
				<div className='text-center space-y-4'>
					...
					<p className='text-vscode-secondary'>Switch to Visual mode to see the workflow canvas</p>
				</div>
			</div>
		);
	}
	...
	return (
		<div className={`flex flex-col h-full ${className}`}>
			...
			>
				{/* Mouse tracking is now handled by React Flow's onPaneMouseMove in WorkflowCanvas */}

				{/* Workflow Canvas */}
				<WorkflowCanvas section={activeSection} className='w-full h-full' />
			</div>
			...
			<div className='flex-shrink-0 bg-vscode-statusBar-background border-t border-vscode-border px-4 py-1'>
				<div className='flex items-center justify-between text-xs text-vscode-statusBar-foreground'>
					<div className='flex items-center space-x-4'>
						...
						{selectedComponents.length > 0 && <span>Selected: {selectedComponents.join(', ')}</span>}
						{/* Mouse coordinates */}
						{(() => {
