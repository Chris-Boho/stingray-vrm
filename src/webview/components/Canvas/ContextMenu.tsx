...
export const ContextMenu: React.FC<ContextMenuProps> = ({
	...
	const { setEditingComponent, getComponentTemplate, createComponent } = useComponentStore();
	const { document } = useDocumentStore();
	const { activeSection } = useEditorStore();
	const { copySelected, pasteAtPosition, canPaste, hasData, info: clipboardInfo } = useClipboardOperations();
	...
	const handleSelectAction = useCallback(
		(action: string, canvasPosition?: { x: number; y: number }) => {
			...
			switch (action) {
				...
				case 'select-all-above':
					...
					const aboveComponentIds = currentSectionComponents
						.filter((c) => c.y < canvasPosition.y)
						.map((c) => c.n);
				...
				case 'select-all-below':
					...
					const belowComponentIds = currentSectionComponents
						.filter((c) => c.y > canvasPosition.y)
						.map((c) => c.n);
		...
		[clearSelection, selectAll, currentSectionComponents, activeSection, store, onClose]
	);
			...
			switch (action) {
				case 'copy':
					if (selectedComponents.length > 0) {
						copySelected();
						console.log(`📋 Copied ${selectedComponents.length} components`);
					}
					break;

				case 'paste':
					if (canPaste && canvasPosition) {
						pasteAtPosition(canvasPosition);
						console.log(`📋 Pasted components at (${canvasPosition.x}, ${canvasPosition.y})`);
					}
					break;
				...
				case 'cut':
					if (selectedComponents.length > 0) {
						...
						console.log(`📋 Cut ${selectedComponents.length} components (delete pending)`);
					}
					break;
			}
			onClose();
		},
		[selectedComponents, copySelected, canPaste, canvasPosition, pasteAtPosition, onClose]
	);

	const handleInsertAction = useCallback(
		(componentType: ComponentType) => {
			if (!canvasPosition || !activeSection) return;
			createComponent(componentType, canvasPosition, activeSection);
			onClose();
		},
		[canvasPosition, activeSection, createComponent, onClose]
	);
	...
	const buildMenuSections = useCallback((): MenuSection[] => {
		...
		const allComponentsSelected =
			selectedComponents.length === currentSectionComponents.length && currentSectionComponents.length > 0;
		...
		mainMenuItems.push({
			...
			icon: (
				<svg className='w-4 h-4' fill='currentColor' viewBox='0 0 20 20'>
					<path
						fillRule='evenodd'
						d='M16.707 5.293a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z'
					/>
				</svg>
			),
			submenu: [
		...
		// Edit menu with enhanced clipboard operations
		mainMenuItems.push({
			id: 'edit',
			label: 'Edit',
			icon: (
			...
			submenu: [
				{
					id: 'edit-cut',
					label: `Cut${hasSelection ? ` (${selectedComponents.length})` : ''}`,
					disabled: selectedComponents.length === 0,
					icon: (
				...
				{
					id: 'edit-copy',
					label: `Copy${hasSelection ? ` (${selectedComponents.length})` : ''}`,
					disabled: selectedComponents.length === 0,
					icon: (
						<svg className='w-4 h-4' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
							<path
				...
				{
					id: 'edit-paste',
					label: 'Paste', //hasData ? `Paste (${clipboardInfo})` : 'Paste',
					disabled: !canPaste || !canvasPosition,
					icon: (
						<svg className='w-4 h-4' fill='none' stroke='currentColor' viewBox='0 0 24 24'>
							<path
								strokeLinecap='round'
				...
				{
					id: 'edit-delete',
					label: `Delete${hasSelection ? ` (${selectedComponents.length})` : ''}`,
					disabled: selectedComponents.length === 0,
					icon: (
						<svg className='w-4 h-4' fill='currentColor' viewBox='0 0 20 20'>
							<path
		...
		mainMenuItems.push({
			...
			submenu: [
				...
				{ id: 'insert-select', label: 'Select Query', onClick: () => handleInsertAction('SELECTQUERY') },
				{
					...
					onClick: () => handleInsertAction('INSERTUPDATEQUERY')
				},
				{ type: 'divider' },
	...
	return (
		...
		>
				...
				{menuSections.map((section, sectionIndex) => (
					<div key={section.title || sectionIndex}>
                      ...
                      `}
                                  ...
                                  ${subItem.id === 'edit-paste' && hasData ? 'font-medium text-blue-400' : ''}
                                `}
															...
															>
																{subItem.icon && (
																	<span className='w-4 h-4 mr-2 flex-shrink-0'>{subItem.icon}</span>
																)}
			...
			{/* Clipboard status indicator at bottom */}
			{hasData && (
				<div className='mt-2 mx-2 pt-2 border-t border-vscode-menu-separatorBackground'>
					<div className='px-2 py-1 text-xs text-vscode-descriptionForeground bg-blue-500/10 rounded flex items-center space-x-2'>
						<svg className='w-3 h-3 text-blue-400' fill='currentColor' viewBox='0 0 20 20'>
							<path d='M8 2a1 1 0 000 2h2a1 1 0 100-2H8z' />
							<path d='M3 5a2 2 0 012-2 3 3 0 003 3h4a3 3 0 003-3 2 2 0 012 2v11a2 2 0 01-2 2H5a2 2 0 01-2-2V5z' />
						</svg>
						<span>📋 {clipboardInfo}</span>
					</div>
				</div>
			)}
		</div>
